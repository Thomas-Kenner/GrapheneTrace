...
/// Updated: SID:2412494 - Converted to IDbContextFactory for thread safety in Blazor Server
/// </summary>
/// <remarks>
...
public class PressureCommentService
{
    ...
    public PressureCommentService(IDbContextFactory<ApplicationDbContext> dbFactory, ILogger<PressureCommentService> logger)
    {
    ...
    public async Task AddCommentAsync(Guid userId, Guid patientId, string commentText, int? sessionId, int? frameIndex)
    {
        ...
        db.PressureComments.Add(comment);
    ...
    /// Author: 2415776
    /// Updated: SID:2412494 - Replaced Console.WriteLine with ILogger, use factory pattern
    /// </summary>
    public async Task<bool> AddReplyAsync(int commentId, string reply)
    {
        await using var db = await _dbFactory.CreateDbContextAsync();
        var comment = await db.PressureComments
    ...
    public async Task<(List<PressureComment> Comments, Dictionary<Guid, string> PatientNames)> GetCommentsForClinicianAsync(Guid clinicianId)
    {
        ...
        var assignedPatientIds = await db.PatientClinicians
        ...
        var comments = await db.PressureComments
        ...
        var patientNames = await db.Users
