...
public class DatabaseSeeder
{
    ...
    public async Task SeedAsync()
    {
        try
        {
            ...
            // Seed chat assignments and messages for the first patient
            if (patients.Count > 0)
            {
                var firstPatient = patients.Values.First();
                await EnsureAssignmentsAndMessagesAsync(firstPatient, clinician);
            }
